...
Point the Pi Camera at any neighborhood. The app streams a live feed directly on screen.

![Demo 1 - Live Camera View](screenshots/demo1.png)

---

### One-Click Capture & Analysis

Press **CAPTURE** to freeze the frame, then **CALCULATE** to run the full albedo analysis pipeline.

![Demo 2 - Capture and Calculate](screenshots/demo2.png)

---

### Full Heat Risk Report

The tool generates a complete one-page report with contour map, coverage chart, heat grade, and city planning recommendations.

![Demo 3 - Full Report Output](screenshots/demo3.png)
...
# Clone the repo
git clone https://github.com/danielge27/raspi-albedo-mapper.git
...
1. Open **Google Earth Pro** (free) on any laptop
2. Navigate to any neighborhood
3. `File → Save → Save Image` at maximum resolution
4. Copy the `.jpg` to the `photos/` folder on your Pi
...
| Surface | Albedo | Heat Risk | Color on Map |
|---------|--------|-----------|--------------|
| Dark Tar Roof | 0.05 | CRITICAL | 🔴 Red |
| Asphalt / Road | 0.10 | HIGH | 🟠 Orange-Red |
| Concrete / Sidewalk | 0.25 | MEDIUM | 🟡 Orange |
| Bright / White Roof | 0.65 | LOW | 🩵 Cyan |
| Trees / Vegetation | 0.20 | BENEFICIAL | 🟢 Green |
| Water Body | 0.06 | COOLING | 🔵 Blue |
...
**From any browser on your WiFi:**
```bash
cd outputs
python3 -m http.server 8080
# Then open: http://YOUR_PI_IP:8080
```
...
**Copy reports to Windows laptop:**
```powershell
...
| `No cameras found` | `sudo raspi-config` → Interface Options → Camera → Enable → Reboot |
| `libcamera: command not found` | Use `rpicam-hello` — newer Pi OS uses `rpicam` not `libcamera` |
| `Permission denied on camera` | `sudo usermod -aG video pi` then reboot |
| Photo is blurry | Increase warmup in `capture.py`: change `time.sleep(3)` to `time.sleep(5)` |
| Report takes too long | Reduce resolution in `capture.py`: change to `resolution=(1920, 1080)` |
| `No module named 'tkinter'` | `sudo apt-get install -y python3-tk` |
